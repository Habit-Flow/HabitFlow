...
class HabitTracker {
    ...
    checkFirstVisit() {
        ...
        if (!hasVisited) {
            ...
            setTimeout(() => {
                this.showToast('👋 Welcome to HabitFlow! Start by adding your first habit above.');
            }, 1000);
        }
    }
    ...
    addHabit() {
        ...
        if (!name) {
            this.showToast('⚠️ Please enter a habit name', 'warning');
            return;
        }
        ...
        if (duplicate) {
            this.showToast('⚠️ This habit already exists', 'warning');
            return;
        }
    ...
    deleteHabit(habitId) {
        ...
        if (confirm(`Are you sure you want to delete "${habit.name}"? This action cannot be undone.`)) {
            ...
            this.showToast(`🗑️ "${habit.name}" deleted`, 'info');
        }
    }
    ...
    toggleHabitCompletion(habitId) {
        ...
        if (selected > today) {
            this.showToast('⚠️ Cannot mark habits for future dates', 'warning');
            return;
        }
        ...
        if (isCompleted) {
            const messages = [
                ...
                '⭐ Excellent! Another day conquered!',
                '🚀 Fantastic! You\'re unstoppable!',
                '🏆 Well done! Success is a habit!',
                '✨ Amazing! Consistency is key!',
            ...
            this.showToast('📝 Habit unchecked', 'info');
        }
    ...
    generateAchievements() {
        const achievements = [
            { id: 'first_habit', name: 'First Step', desc: 'Add your first habit', icon: '🌱', unlocked: this.habits.length > 0 },
            { id: 'five_habits', name: 'Habit Builder', desc: 'Add 5 habits', icon: '🏗️', unlocked: this.habits.length >= 5 },
            { id: 'ten_habits', name: 'Habit Master', desc: 'Add 10 habits', icon: '🎯', unlocked: this.habits.length >= 10 },
            { id: 'week_warrior', name: 'Week Warrior', desc: '7 day streak', icon: '🗓️', unlocked: this.hasStreak(7) },
            { id: 'fortnight', name: 'Fortnight Fighter', desc: '14 day streak', icon: '💪', unlocked: this.hasStreak(14) },
            { id: 'monthly', name: 'Monthly Champion', desc: '30 day streak', icon: '👑', unlocked: this.hasStreak(30) },
            { id: 'quarterly', name: 'Quarter Master', desc: '90 day streak', icon: '🏅', unlocked: this.hasStreak(90) },
            { id: 'yearly', name: 'Year Legend', desc: '365 day streak', icon: '🌟', unlocked: this.hasStreak(365) },
            { id: 'perfectionist', name: 'Perfectionist', desc: '100% daily completion', icon: '💯', unlocked: this.hasPerfectDay() },
            { id: 'dedicated_10', name: 'Dedicated', desc: '10 total completions', icon: '📍', unlocked: this.getTotalCompletions() >= 10 },
            { id: 'committed_50', name: 'Committed', desc: '50 total completions', icon: '🎖️', unlocked: this.getTotalCompletions() >= 50 },
            { id: 'champion_100', name: 'Champion', desc: '100 total completions', icon: '🏆', unlocked: this.getTotalCompletions() >= 100 },
            { id: 'legend_500', name: 'Legend', desc: '500 total completions', icon: '🔥', unlocked: this.getTotalCompletions() >= 500 },
            { id: 'explorer', name: 'Explorer', desc: 'Try 5 categories', icon: '🚀', unlocked: this.getUniqueCategories() >= 5 },
            { id: 'diverse', name: 'Diverse', desc: 'Try 10 categories', icon: '🌈', unlocked: this.getUniqueCategories() >= 10 }
        ];
    ...
    toggleTheme() {
        ...
        this.showToast(isDarkMode ? '🌙 Dark mode enabled' : '☀️ Light mode enabled', 'info');
    }
    ...
    clearAllData() {
        if (confirm('⚠️ This will delete all your habits and progress. This action cannot be undone. Are you sure?')) {
            if (confirm('Are you really sure? All your data will be lost forever!')) {
                ...
                this.generateAchievements();
                this.updateProgressCharts();
                this.showToast('🗑️ All data has been cleared', 'info');
            }
        }
    }
    ...
    editHabit(habitId) {
        ...
        this.showToast('📝 Edit your habit and save', 'info');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('%c Built with ❤️ for better productivity ', 'color: #6366f1; font-size: 12px;');
});
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        ...
        console.log('HabitFlow is ready! 🚀');
    });
}
